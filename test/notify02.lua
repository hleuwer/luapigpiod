...
print("Register ping callback func ...")

local cb, err = assert(sess:callback(pinp, gpio.EITHER_EDGE, alert))
printf("  callback ID: %d", cb.id)

printf("Register event callback func for event %d ...", eventindex)
...
for i = 1, N do
   ...
   printf("Trigger Event %d...", eventindex)
   sess:triggerEvent(eventindex)
   --   print("set 0")
   if bitmode == "yes" then
      sess:write(pout, 0)
   else
